...
export function parseRange(value: string | undefined): RangeKey {
  return value === "week" || value === "month" || value === "year" || value === "all" ? value : "week";
}

export function filterRecordsByRange(records: DailyRecordFull[], range: RangeKey): DailyRecordFull[] {
  const now = new Date();
