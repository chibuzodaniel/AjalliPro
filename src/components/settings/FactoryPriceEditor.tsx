...
import { updatePricingSettings } from "@/app/(app)/settings/actions";

export default function FactoryPriceEditor({ initial }: { initial: number }) {
  ...
  async function handleSave() {
    ...
    setError(null);
    setSaved(false);
    const result = await updatePricingSettings({ factoryPricePerBag: Number(price) || 0 });
    setLoading(false);
    if (!result.ok) {
  ...
  return (
    <div>
      <div className="field">
        <label>Factory sale price (₦ per bag)</label>
