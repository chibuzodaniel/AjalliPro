...
export default function DeleteCustomerButton({ id, name }: { id: string; name: string }) {
  ...
  return (
    <>
      ...
      {open && (
        <Modal open={open} onClose={() => setOpen(false)} title="Delete customer?">
          <div style={{ display: "flex", flexDirection: "column", gap: 12, fontSize: 13.5 }}>
            <p style={{ margin: 0 }}>
              This will <b>permanently delete</b> <b>{name}</b>. This cannot be undone. Customers linked to any
              sale can't be deleted — remove or edit those daily records first.
            </p>
